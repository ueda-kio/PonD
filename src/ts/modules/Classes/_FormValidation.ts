import { submitContact } from "../_submitContact";

type TextField = HTMLInputElement | HTMLTextAreaElement;
type FormControls = TextField | HTMLSelectElement;

export class Validation {
	private form: HTMLFormElement;
	private isInvalid = new Map();
	private inputs: NodeListOf<HTMLElement>;
	private validate = {
		/**
		 * 文字数が足りているか
		 ...
		 * @returns エラーメッセージ
		 */
		minLength: (_input: FormControls) => {
			if (_input.getAttribute('minLength') === null) return '';

			const minlength = Number(_input.getAttribute('minlength'));

			return minlength <= _input.value.trim().length ? '' : `${minlength}文字以上入力してください。`;
		},
		 ...
		 * @returns エラーメッセージ
		 */
		maxLength: (_input: FormControls) => {
			if (!_input.getAttribute('maxLength')) {
				return '';
			}

			const input = _input as TextField;
		 ...
		 * @returns エラーメッセージ
		 */
		required: (_formControls: FormControls) => {
			if (!_formControls.required) {
				return '';
			}

			const input = _formControls as HTMLInputElement;

			if (input.type === 'checkbox') {
				return input.checked ? '' : 'このフィールドは入力必須です。';
			} else if (input.type === 'radio') {
				const isChecked = this.form.querySelectorAll(`input[type="radio"][name="${input.name}"]:checked`).length !== 0;
		 ...
		 * @returns エラーメッセージ
		 */
		pattern: (_input: FormControls) => {
			if (!_input.getAttribute('pattern') || !_input.required) {
				return '';
			}

			const input = _input as HTMLInputElement; // pattern属性を許容させる
	...
	constructor(form: HTMLFormElement) {
		...
		this.inputs = form.querySelectorAll<FormControls>('input, textarea, select');
		 ...
		 * @returns エラーメッセージ
		 */
		const getErrorId = (input: FormControls) => {
		 ...
		 * @returns {void}
		 */
		const resetInputErrorState = (input: FormControls) => {
			const type = input.type;
			const inputElms = type === 'radio' ? form.querySelectorAll<HTMLInputElement>(`input[type="radio"][name="${input.name}"]`) : [input];
		...
		const getHandler = (input: FormControls) => {
			const errorId = getErrorId(input);
			const error = document.getElementById(errorId);
			const labels = document.querySelectorAll(`[for="${input.id}"]`);
			let isValid = true;
		...
		const addEvents = () => {
			...
			this.inputs.forEach((input) => {
				input.addEventListener('input', (e) => {
					getHandler(<FormControls>e.currentTarget);
				})
			})
...
document.querySelectorAll<HTMLFormElement>('.js-form-validate').forEach((el) => {
	new Validation(el);
})
