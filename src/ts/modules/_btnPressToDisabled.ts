import { ARIA_DISABLED_ELEMS, DISABLED_ELEMS } from "./utils/constants";
...
export const btnPressToDisabled = () => {
   ...
   */
  const isDisabled = (elem: HTMLSelectElement) => {
    if (
   ...
   */
  const addDisabled = (elem: HTMLSelectElement) => {
  ...
  const toDisable = () => {
    document.querySelectorAll<HTMLElement>('.js-btn-press-to-disabled').forEach((btn) => {
      if (btn) {
        btn.addEventListener('click', (e) => {
          if (isDisabled(<HTMLSelectElement>e.currentTarget)) {
            e.preventDefault();
            e.stopPropagation();
          ...
          addDisabled(<HTMLSelectElement>e.currentTarget);
        });
      }
    });
  };
